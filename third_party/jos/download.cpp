...
*/
vector<uint8_t> Download::download(const char *url, bool reload, void (*update)(unsigned long, unsigned long))
{
        ...
        */
        ...
        if(filelen != contentlen && contentlen)
        {
            do
            {
                ...
                for(int i=0; i<numrcved; i++) vec.push_back(buf[i]);
...
unsigned long Download::openfile(char *url, bool reload, ofstream &fout)
{
    ...
    if(!reload)
        ...
        fout.open("temp.$$$", ios::binary | ios::out | ios::app | ios::ate);
    else
        //fout.open(fname, ios::binary | ios::out | ios::trunc);
        fout.open("temp.$$$", ios::binary | ios::out | ios::trunc);
